...
\begin{proof}
  ...
  %$\seqt_j = l_{j,1} \cdot \seqt_1 + … + l_{j,I} \cdot \seqt_I + \labt{\treepath[n^\omega_I]{n}}$
  ...
  \todo{Donc Tts(m,n) = see notes "rep3"}
%
%
% 
%
...
%  \end{itemize}
%
  %  For any node $n \in \nodes$ let $\zeta(n) = \op{\lab{\parent{n}}}$ be the set of \oplaces that were already accelerated in $\lab{\parent{n}}$, and
%
%
%
%  Thus we have for any node $n \in \nodes$ three disjoint sets of places whose the union is $\places$:
%  \begin{itemize}
...
%
%
  %  If $n$ is the root of the Karp and Miller tree, \mar is covered by $\lab{n_0}$ and thus $\tts{n}{\mar}$ is the empty sequence:
  %  \( \tts{n}{\mar} = () \text{ whenever } n = n_0 \).
%  This is the base case.
%
  %  If $n \neq n_0$, we have $\tts{n}{\mar} = \tts{\parent{n}, \mar''} + \labt{n} + \labt{s_I}$ for well chosen marking $\mar'' \in \downc{\lab{\parent{n}}}$ and sequence of nodes $s_I$.
%
%
%
%
...
%  Thus, the sequence comes directly from $\treepath{n}$.
  %  \( \tts{n}{\mar} = \labt{\treepath{n}} \text{ whenever } \op{\lab{n}} = \emptyset \).
%  %Note that the previous case is a special case of this one.
  %  In a recursive way, this can be expressed as follows: $\tts{n}{\mar} = \tts{\parent{n}, \lab{\parent{n}}} + \labt{n} \text{ whenever } \op{\lab{n}} = \emptyset$.
  %  %\( \tts{n}{\mar} = \labt{\treepath{n}} \text{ whenever } \nexists p \in \places \text{ s.t. } \lab{n}(p) = \omega \)
  %  %\[ \tts{n}{\mar} = \Tts\big(\places(n),\lab{\places(n)}\big) + (\labt{n}) = \labt{\treepath{n}} \text{ whenever } \nexists p \in \places \text{ s.t. } \lab{n}(p) = \omega \]
%
...
%  that is $\zeta(n) = \emptyset \wedge \alpha(n) \neq \emptyset$,
  %  we construct $\tts{n}{\mar} = \seqt$ as follows.
%  (Here are the ideas, we express them more precisely and in a recursive way right after.)\\
  ...
  %  Thus $\labt{\treepath[n']{n}}$ is an increasing self covering sequence
%  and
%  there exists some $k \in \naturals$ and $\mar' \in \naturals^{\card{\places}}$, $\mar \preceq \mar'$
%  such that
  %  $\seqt = \labt{\treepath{n'}} + k \cdot \labt{\treepath[n']{n}}$ and $\mari \fire{\seqt} \mar'$. % with $\mar \preceq \mar'$.
  %  More precisely, one can choose any $k$ that verifies $k \geq \left\lceil\frac{\mar(p) - \lab{n'}(p)}{\effect{\labt{\treepath[n']{n}}}}\right\rceil$. %, but lower $k$ may also exists \todo{see notes +1+}.
%  Note that $k > 0$.\\
  %  Second, if there exists many \oplaces $p_i$ in $\lab{n}$, one can compute $\seqt$ by considering the different $p_i$ one by one:
  %  We obtain for $p_1$ the sequence $\seqt_1 = \labt{\treepath{n_1'}} + k_1 \cdot \labt{\treepath[n_1']{n}}$ as above.
...
%  Let us now express this in a more formal and recursive way.\\
  %  Since $\zeta(n) = \emptyset$, the previous case give us $\tts{\parent{n}, \lab{\parent{n}}}$.
  %  We denote by $s_0$ the sequence of nodes used to obtain $\tts{\parent{n}, \lab{\parent{n}}}$.
  %  Here this sequence is $\treepath{\parent{n}}$, but for the cases that follow, let's keep in mind that $s_0$ is the sequence of nodes used to obtain $\tts{\parent{n}, \mathit{previous\_marking}}$, whatever $\mathit{previous\_marking}$ is.\\
...
%  This is to consider the longest self increasing covering sequences first and thus obtain the shortest sequence $\seqt$ \todo{to prove}.\\
  %  We compute the sequence of nodes $s$ such that $\seqt = \labt{s} = \labt{s_0 + (n) + s_I}$.
...
%    \begin{cases}
%      \left\lceil
  %        \frac{\mar(p_i) - \mari(p_i) - \effect{\labt{s_0+(n)}}(p_i)}
  %            {\effect{\labt{\treepath[n_i]{n}}}}
%      \right\rceil
%      \cdot \treepath[n_i]{n}
%      &\text{ if } i = 1\\
%      \\
%      \max\left(0,
%        \left\lceil
  %          \frac{\mar(p_i) - \mari(p_i) - \effect{\labt{s_0+(n)+s_{i-1}}}(p_i)}
  %              {\effect{\labt{\treepath[n_i]{n}}}}
%        \right\rceil
%      \right)
%      \cdot \treepath[n_i]{n}
...
%  \]
%
  %So we have $\tts{n}{\mar} = \tts{\parent{n}, \lab{\parent{n}}} + \labt{(n) + s_I}$ whenever $\zeta(n) = \emptyset \wedge \alpha(n) \neq \emptyset$.
%
  %  %Since the sequences $s_i$ thus obtained begin with $\treepath{n}$, $\tts{n }{\mar}$ can be expressed in a recursive form, but it is more complicated than necessary to prove its existence.
%  %\todo{see notes +1+}
%
%
%
...
%  %since the Karp and Miller algorithm never removes $\omega$s,
  ...
  %  One can easily compute the lowest marking $\mar''$ in $\downc{\lab{\parent{n}}}$ such that $\mar'' \fire{\labt{n}} \mar'$ with $\mar \preceq \mar''$:
%  \[
%    \mar''(p) = \begin{cases}
  %      \lab{\parent{n}}(p)
  %        &\text{ if } p \notin \op{\lab{\parent{n}}} \\
  %      \max{\mar(p) - \effect{\labt{n}}(p), I_{\labt{t}}(p)}
%        &\text{ otherwise}
%    \end{cases}
%  \]
%
  %  %We show that there exists a marking $\mar'' \in \downc{\lab{\parent{n}}}$ such that
  %  %(1) $\labt{n}$ is enabled in $\mar''$
%  %and that
  %  %(2) $\mar'$ given by $\mar''\fire{\labt{n}}\mar'$ is such that $\mar \preceq \mar'$.\\
...
%  %  \mar''(p) = \begin{cases}
  %  %    \lab{\parent{n}}(p)
  %  %      &\text{ if } p \notin \op{\lab{\parent{n}}} \\
  %  %    c \text{ with } c \geq \max{\mar(p) - \effect{\labt{n}}(p), I_{\labt{t}}(p)}
%  %      &\text{ otherwise}
%  %  \end{cases}
%  %\]
%  Indeed, %(1)
...
%  $\mar \preceq \mar'$:
  %  for the \noplaces $p$ of $\lab{\parent{n}}$, we have $\mar'(p) = \mar''(p) + \effect{\labt{n}}(p) = \lab{n}(p) \geq \mar(p)$ by choice of $n$ and \mar;
  %  and for the \oplaces $p$ of $\lab{\parent{n}}$, we have $\mar'(p) = \mar''(p) + \effect{\labt{n}}(p) \geq \mar(p)$ by construction of $\mar''$.\\
  %  Thus, $\tts{n}{\mar} = \tts{\parent{n}, \mar''} + \labt{n}$
%  whenever
%  $\alpha(n) = \emptyset \wedge \zeta(n) \neq \emptyset$.
  %  %This allows to define $\tts{n}{\mar}$ recursively up on the branch.
%
  %  %We first compute a marking $\mar'' \in \downc{\lab{n''}}$ such that $\labt{\treepath[n'']{n}}$ is enabled in $\mar''$, then we show that $\mar'$ given by $\mar''\fire{\labt{\treepath[n'']{n}}}\mar'$ is such that $\mar \preceq \mar'$.\\
%  %\[
%  %  \mar''(p) =
%  %  \begin{cases}
  %  %    \lab{n''}(p)
  %  %      &\text{ if } \lab{n''}(p) \neq w \\
  %  %    \mar(p) - \min(\effect{\labt{\treepath[n'']{n}}}, \max\I(\labt{n_i})
  %  %      &\text{ otherwise and if } \effect{\labt{\treepath[n'']{n}}} < 0
%  %  \end{cases}
%
...
%  %The base case is the one treated above, where $\omega$ are first introduced.
  ...
  %  %And so we have $\tts{\mar}{n} = \tts{\mar''}{\parent{n}} + (\labt{n})$.
%
%  \todo{Formally}
%
...
%  Actually, all the cases studied above are special cases of this one.
  %  Here, one can compute $\tts{n}{\mar}$ by combining the different constructions above:
...
%  Compute $k_1 = $ \todo{} and for $i \neq 1, k_i = $\todo{}.
  %  Then, it is easy to see that $\tts{n}{\mar} = \tts{\parent{n}, \mar''} + \labt{n} + k_1 \dot \labt{\treepath[n_1]{n}} + k_2 \dot \labt{\treepath[n_2]{n}} + …$
\end{proof}
...
%To prove that the Karp and Miller Tree $\nameT$ is a coverability tree of \namePN, we show
%\begin{enumerate}
%  \item \todo{} that one can construct a sequence from \mari to any marking \mar “covered by the tree” given a node $n$ such that $\mar \in \downc{\labt{n}}$, and
...
%  For this purpose, we introduce some notations:
%  \begin{itemize}
%    \item we attach to the Karp and Miller tree $\nameT$ the mapping $\Labt : \nodes \setminus \{n_0\} \mapsto \transitions$ that gives for all node of the tree (but the root) the transition used to create it at step (2) of the Karp and Miller algorithm.
%
%      By abuse of notation, we will note $\labt{(n_1, n_2, ..., n_m)}$
%      %with $n_i, i \in \{1, ..., m\}$ some nodes of $\nameT$
%      with  $n_i \in \nodes$
...
%
%  We now recursively show that $\tts{\mar}{n}$ exists for all such $\mar, n$ pair.
%
%  If $n$ is the root of the Karp and Miller tree, \mar is covered by $\lab{n_0}$ and thus $\tts{n}{\mar}$ is the empty sequence:
%  \( \tts{n}{\mar} = () \text{ whenever } n = n_0 \).
%  This is the base case.
