...
It is called the covering set of \tupleN and is noted $\Cover(\tupleN)$.
...
Symmetrically, any downward closed set may be represented by a finite set of \omarks. \todo{Indeed...}
...
To keep $N$ finite, the Karp and Miller procedure exploits the strong monotonicity of \acp{PN} to introduce \omark{}s through an \emph{acceleration function} $\KMAcc$.
...
\ac{EEC}, introduced in \citep{Geeraerts07thesis, Geeraerts06}, is an iterative algorithm that allows, among other, to solve the coverability problem for \acp{PN}.
We present it restricted to this context, but it may be used for a wide range of well structured transitions systems, which \acp{PN} is part of, because it relies only on the monotonicity, and not on the strict monotonicity of these models.
...
We define $(C_i)_{i \geq 0}$ to be the sequence of set of markings holding no more than $i$ tokens in each places (plus \marqi):
\[
  \forall i \in \mathbb{N} : C_i = \{0, ..., i\}^{|P|} \cup \{\marqi\}
\]
At step $i$, the algorithm computes $\Sous(\tupleN, C_i)$ defined as the graph $\langle C_i, \marqi, \sousrel \rangle$ where 
