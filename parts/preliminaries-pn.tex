...
\begin{proof}
  ...
  Now considers the case where $\ab{n} \neq \emptyset$.
  ...
  \[
    \tts{\mar}{n} = \labt{\treepath{n_1}} + k_1 \cdot \iscs{1} + \dots + k_J \cdot \iscs{J} + \labt{\treepath[n^\omega_J]{n}}
  \]
  where each increasing self-covering sequences $\iscs{j}$ is computed in the context of first node of the branch where it is accelerated: $n^\omega_j$,
  ...
  \( \mar'' \fire{\labt{\treepath[n^\omega_J]{n}}} \mar' \). \qedhere
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item otherwise, for each place $p$ such that $\mar_i(p) < \mar_j(p)$, replace $\mar_k(p)$ by $\omega$ for all the markings $\mar_k$ from $\mar_j$ (included) to the end of the sequence.
  \end{itemize}
...
\begin{proof}
  This follows from \autoref{lemm:wqo}.
\end{proof}
