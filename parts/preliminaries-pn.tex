...
\ac{EEC}, introduced in \citep{Geeraerts07thesis, Geeraerts06}, is an iterative algorithm that allows, among other, to solve the coverability problem for \ac{PN}.
...
This ensure the termination of the algorithm.

Indeed, let $S$ be the set of marking we wants to cover and let $U$ be $\upc(S)$.
If $U$ is reachable, we will eventually get a $C_i$ that contains all the markings of a path from $\marqi$ to $U$.
As this path will be present in $\Sous(\tupleN, C_i)$, we will have that $\R(\Sous(\tupleN,C_i)) \cup U \neq \emptyset$.\\
Symmetrically, if $U$ is not reachable, we know that $\downc(\R(\Sur(\tupleN,L_i))) \cap U = \emptyset$ will eventually happen because, as for a $j$ great enough, $j \geq i$, $L_j$ will contains the cover set of \tupleN, we will have that $\downc(\R(\Sur(\tupleN,L_j))) = \Cover(\tupleN)$.
