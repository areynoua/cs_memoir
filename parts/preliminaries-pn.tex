...
\begin{defi}[Coverability set \citep{Finkel87,Finkel90}]
  Given an initialized \ac{PN} $\PN = \PTm$, a \emph{coverability set} $S$ of \PN is a set of markings such that $\downc(S) = \downc(\Post^*(\mari))$.
\end{defi}

Obviously, the minimal coverability set is $\downc(\Post^*(\mari))$.
It is called the \emph{covering set} of \PN and is noted $\Cover(\PN)$.

\begin{defi}[Covering set]
  Let $\PN = \PTm$ be an initialized \ac{PN}. The \emph{covering set} $S$ of \PN, noted $\Cover(\PN)$, is the set $\{c \mid \exists c' \in \Post^*(\PN) : c \leq c' \}$.
\end{defi}
...
\begin{defi}[Coverability problem]
  ...
  Given a \ac{PN} \PN and an upward-closed set $U = \upc(S)$ of markings of \PN, detemine whether $\Post^*(\mari) \cap U = \emptyset$.
\end{defi}
...
\begin{defi}[Self-covering sequence]
  Given an initialized \ac{PN} $\PN = \PTm$,
...
Having an \omark $\mar \in \Cover(\PN)$ denotes that for all marking $\mar_1$ such that $\mar_1(p) = \mar(p) \forall p \in \{p \mid \mar(p) \neq \omega\}$, there exists a marking $\mar_2$ in the reachability set of \PN such that $\mar_1 \prec \mar_2$.
...
\begin{defi}[Coverability tree]
  Given a \ac{PN} $\PN = \PTm$, a coverability tree $\mathcal{T}$ of \PN is a labelled tree $\mathcal{T} = \langle N, B, n_0, \Lambda\rangle$ where:
  \begin{itemize}
    \item $N$ is the set of nodes of the tree.%, is a set of \omark of \PN such that $\downc(N) = \Cover(\PN)$.
    \item $n_0 \in N$ is the root of the tree, \lang{i.e.} $\nexists n \in N$ such that $(n, n_0) \in B$.
    \item $\Lambda : N \mapsto (\mathbb{N} \cup \{\omega\})^{|P|}$ is a labelling function that associate to each node a \textomega-marking of \PN.
    \item $B \subseteq N \times N$, the set of edges, is such that:
      ...
      \end{itemize}
  \end{itemize}
  and such that $\downc(\{\Lambda(n) \mid n \in N\}) = \Cover(\PN)$.
\end{defi}
...
This relationship is of interest because it ensures that there exists a sequence $\sigma$ of transitions such that $\mar_1 \fire\sigma \mar_2$.
If, in addition, $\mar_1 \prec \mar_2$, we know than an acceleration may occur.
...
\begin{itemize}
  \item by monotonicity, all potential successor of the elements of this pair will be covered by successor of $\mar_1$ or $\mar_2$, and
...
The efficient computation of the coverability set of the marked \ac{PN} $\NPTm$ lies on the sequence $\CovSeq(\PN) = (V_i)_{i \geq 0}$ of pair of \omarks, where, for all marked \ac{PN} $\PN$ we have:
\begin{gather*}
...
second, all the markings produced by $\Postb$ and $\Accelb$ are in the coverability set of \PN.
\todo{Indeed...}

These two results lead us to the following lemma:
\begin{lemm}[\cite{Geeraerts07}]
  Given a marked \ac{PN} \PN such that $\CovSeq(\PN) = (V_i)_{i \geq 0}$,
  there exists $k \geq 0$ such that for all $l \in \{0, ..., k-1\}$ we have that $\downc(\Flatten(V_l)) \subset \downc(\Flatten(V_{l+1}))$
  and for all $l \geq k : \downc(\Flatten(V_l)) = \Cover(\PN)$.
\end{lemm}
...
Thus, one can compute $\Cover(\PN)$ of a \ac{PN} \NPTm by computing the sequence $(V_i)_{i \geq 0}$ defined below until $\downc(\Flatten(V_i)) = \downc(\Flatten(V_{i-1}))$.
\begin{gather*}
  V_0 = \{(\mari, \mari)\} \text{ and } \\
  \forall i \geq 1 : V_i = \Maxs(V_{i-1} \cup \Postb(V_{i-1}) \cup \Accelb(V_{i-1}))
\end{gather*}

At the end, we have that $\downc(\Flatten(V_i)) = \Cover(\PN)$.
...
We define $(C_i)_{i \geq 0}$ to be the sequence of finite set of markings holding no more than $i$ tokens in each places (plus \mari):
\[
  \forall i \in \mathbb{N} : C_i = \{0, ..., i\}^{|P|} \cup \{\mari\}
\]
At step $i$, the algorithm computes $\Sous(\PN, C_i)$ defined as the graph $\langle C_i, \mari, \sousrel \rangle$ which is the transition system induced by the \ac{PN} \PN restricted to the markings of $C_i$, \lang{i.e.} $(\mar_1, \mar_2) \in \sousrel$ if, and only if, $\mar_1 \rightarrow \mar_2$.
The under-approximation sought is the set of markings reachable through $\sousrel$ from \mari and is denoted $\R(\Sous(\PN, C_i))$.
...
This set is used to constructs the graph $\Sur(\PN, C_i)$ defined as the graph $\langle L_i, \mari, \surrel \rangle$ where $(\mar_1, \mar_2) \in \surrel$ if, and only if:
\begin{itemize}
...
The over-approximation, then, is the set of markings of $L_i$ reachable through $\surrel$ from \mari. It is denoted $\R(\Sur(\PN, L_i))$.
...
\begin{lemm}[Under-approximation \cite{Ganty09}]
  For all \ac{PN} \NPTm, for all upward-closed set $U \subseteq \mathbb{N}^{|P|}$, and for all $i \geq 0: \R(\Sous(\PN,C_i)) \cap U \neq \emptyset \Rightarrow \Post^*(\mari) \cap U \neq \emptyset$.
\end{lemm}
\begin{lemm}[Over-approximation \cite{Ganty09}]
  For all \ac{PN} \NPTm, for all upward-closed set $U \subseteq \mathbb{N}^{|P|}$, and for all $i \geq 0: \downc(\R(\Sur(\PN,L_i))) \cap U = \emptyset \Rightarrow \Post^*(\mari) \cap U = \emptyset$.
\end{lemm}
...
As this path will be present in $\Sous(\PN, C_i)$, we will have that $\R(\Sous(\PN,C_i)) \cap U \neq \emptyset$.\\
Symmetrically, let $j$ be such that $L_j$ contains the maximal elements of the cover set of \PN.
Such a $j$ exists and we have that $\downc(\R(\Sur(\PN,L_j))) = \Cover(\PN)$.
Thus we know that $\downc(\R(\Sur(\PN,L_i))) \cap U = \emptyset$ will eventually happen for a $i \leq j$.


\removed{A backward algorithm \citep{Finkel90, Abdulla96}}
%\subsection{A backward algorithm \citep{Finkel90, Abdulla96}}
%
%We will now present an algorithm to solve the coverability problem for a marking \mar of a \ac{PN} $\PN = \PTm$.
%
